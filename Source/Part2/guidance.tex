...
\begin{itemize}
    ...
    \item The \index{Artefact, Data}\gls{data artefact} holds one or more of the properties\index{Integrity!Property}\gls{integrity}, \index{Completeness!Property}\gls{completeness} or \gls{verifiability}\index{Verifiability Property} (``I'', ``C'' or ``V'' from \autoref{tab:Categories}).
\end{itemize}
...
\begin{longtable}
  ...
  \hline
  DD.11 & Outlier analysis in design\hl{Do we need to say ``in design'' since this is in DD?} & \dsiwgTextTT{..D..} & - & R & R & HR & Use of machine learning training data containing edge / corner cases, boundary
  cases, and testing well beyond normal or acceptable ranges to give insight into the system
  degradation behaviour. & \dsiwgTextTT{IC.YOARTMVL.PQU..HE.GZXK}\\
  \hline
  DD.12 & Outlier analysis in verification & \dsiwgTextTT{V....} & - & R & R & HR & Data used for such image recognition systems \hl{What image recognition systems? We've not mentioned them.}filters and excludes potentially
  dangerous extraneous data arising from unanticipated situations. & \dsiwgTextTT{IC.YOARTMVL.PQU..HE.GZXK}\\
  \hline
...
\begin{description}
	...
	\item[Note 2] For verification data categories,
...
\item[Note 5] For justification data categories, the use of dialectic
...
\item[Note 6] For verification data categories, comparisons,
