...
%================================================================================
\chapter{Tool Confidence and Tool Qualification of Data Processing Tools}\index{Tools}\label{bkm:tools}
%
...
or the application of ``tool qualification'' to ensure that a lack of \gls{integrity}\index{Integrity} in the tools does not impact  the safety of the product. 
...
\item Consider all outputs of the tools.
  ...
  \item The achieved \gls{accuracy}\index{Accuracy Property}\index{Data Property!Accuracy}.
  \end{enumerate}
...
consider every new application as a new use of the tool -- in other words, classification of a tool can never be truly generic, but must be reviewed for each specific use. 
  An example of failing to carry out this reassessment is the Covid Summary Table (see Part 3 Section~\ref*{Part3-bkm:incacc:covidexcel}),
...
\item When looking to the risks of tools, it is not sufficient to consider only the functional risks like ``wrong data created''
  ...
  (\dsiwgExtRef{Subsection}{3}{bkm:issues} Ways that Data Can Cause  Problems) should be considered for impact from the tools.
  ...
  \begin{itemize}
  \item Each issue listed in \dsiwgExtRef{Subsection}{3}{bkm:issues}; and
  \item Each Property listed in \dsiwgExtRef{Table}{1}{tab:PropertiesOfData}
  \end{itemize}    
\end{itemize}
