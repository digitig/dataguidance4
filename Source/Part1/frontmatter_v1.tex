...
\href{https://scsc.uk/events-diary/safety-critical-systems-club-symposia/safety-critical-systems-symposium-2026}{https://scsc.uk/events-diary/safety-critical-systems-club-symposia/safety-critical-systems-symposium-2026}%\todo{Update e797 reference}.
...
\cbstart This is a major rewrite of the guidance. The document has been split into three volumes , each giving progressively more detail.

The process remains substantially the same. The intention is that compliance with this version does imply compliance with earlier versions \hl{???Need to check this is still true}, but some material that was informative\index{Informative Text} in earlier versions has been made normative\index{Normative Text}, so compliance with earlier versions of the guidance does not guarantee compliance with this version.\cbend\
...
\hl{???Update future work}
\hl{???Consider whether future work needs to be included in all parts}
...
These improvements are intended to clarify the application of the data safety process\hl{???Review future work}
and include:
\begin{itemize}
...
The use of \gls{ai} systems is a new, exciting, but potentially hazardous use of data. \Gls{llm} \todo{Also true of ML. Mention that?} based systems are trained on vast amounts of data, and it is this data which enables them to be useful.
...
When reference is made to the content of other volumes, this is indicated by the volume number shown in bold text. So ``\dsiwgExtRef{Chapter}{3}{bkm:accidents}'' refers to chapter \ref*{Part3-bkm:accidents} in volume 3.
