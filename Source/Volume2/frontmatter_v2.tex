...
\cbstart\gls{sss}'26, 10--12 February 2026\cbend\
details of which may be found at\hl{???Update reference to SSS'26}
\href{https://scsc.uk/e1099}{https://scsc.uk/e1099}%\{Update e797 reference}.

Comments on this document are actively encouraged. These can be emailed to \href{mailto:comments@data-safety.scsc.uk}{comments@data-safety.scsc.uk}.
...
\cbstart This is a major rewrite of the guidance. The document has been split into three volumes.

The process remains substantially the same. The intention is that conformance with this version does imply conformance with earlier versions, but some informative material has been made normative so conformance with earlier versions of the guidance does not guarantee conformance with this version.\cbend\

\subsection*{Future work}\hl{???update future work}
...
\section*{Foreword}\hl{???consider whether foreword needs to be repeated in all parts}
...
\clearpage
\section*{Quick Start Guide}\hl{???consider whether the quick start guide needs to be included in all parts}
...
\begin{description}
	\item[Volume 1] contains the normative material. It describes the ``what'' of the guidance and states what is needed to ensure conformance with this guidance, if required. It should be of use to those establishing processes and procedures for data safety assurance and for those doing the actual assurance;
	\item[Volume 2] (this volume) contains the informative material. It describes the ``how'' of the guidance and should be of particular value to data safety practitioners;
...
\cbstart
\begin{itemize}
	\item The underlying principles of the guidance are described in \dsiwgExtRef{Chapter}{1}{Part1-bkm:principles}.
	\item The objectives and outputs to satisfy the principles are described in \dsiwgExtRef{Chapter}{1}{Part1-bkm:objectives}.
	\item Normative definitions and abbreviations are described in \dsiwgExtRef{Chapter}{1}{Part1-bkm:definitionsabbreviations}.
	...
	\item A worked example is provided in Chapter \dsiwgRef{Chapter}{bkm:workedexample}.
	\item Guidance for the assignment of \glspl{dsal} is given in \dsiwgRef{Chapter}{bkm:DsalCustomisation}.
	\item Examples of ways that \glspl{dsal} may be customised, with particular focus on likelihood are given in \dsiwgRef{Chapter}{bkm:tools}.
	...
	\item Additional information on lifecycle data categories \todo{We never seem to explain what we mean by lifecycle data categories. Consider doing so.}is given in \dsiwgRef{Chapter}{Part2-bkm:categories}.
