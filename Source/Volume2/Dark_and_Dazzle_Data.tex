...
%
%================================================================================
\chapter{Dark Data\index{Data!Dark|see {Dark Data}} and Dazzle Data\index{Data!Dazzle|see {Dazzle Data}}}%
...
\subsection{Dark Data Varieties and Safety Examples}
...
\subsubsection{Data We Know Are Missing: ``Known Unknowns''}\label{bkm:dark1}
...
\subsubsection{Data We Don’t Know Are Missing: ``Unknown unknowns''}\index{Unknown Unknowns}\label{bkm:dark2}
...
\begin{itemize}
  \item The Covid-19 Track and Trace data loss (Part 3 Section~\ref*{Part3-bkm:incacc:covidexcel}), where the organisation handling the data was unaware that rows were missing from a spreadsheet for some time is illustrated in \autoref {fig:darknewcases}
...
\subsubsection{Choosing Just Some Cases}\label{bkm:dark3}
...
\subsubsection{Self-Selection}\label{bkm:dark4}
...
\subsubsection{Missing What Matters}\label{bkm:dark5}
...
\subsubsection{Data Which Might Have Been}\label{bkm:dark6}
...
\subsubsection{Changes with Time}\label{bkm:dark7}
...
\subsubsection{Definitions of Data}\label{bkm:dark8}
...
\subsubsection{Summaries of Data}\label{bkm:dark9}
...
\subsubsection{Measurement Error and Uncertainty}\label{bkm:dark10}
...
\subsubsection{Feedback and Gaming}\label{bkm:dark11}
...
\subsubsection{\glsfmttext{information} Asymmetry}\label{bkm:dark12}
...
\subsubsection{Intentionally Darkened Data}\label{bkm:dark13}
...
\subsubsection{Fabricated and Synthetic Data}\label{bkm:dark14}
...
\subsubsection{Extrapolating Beyond Your Data}\label{bkm:dark15}
...
\subsection{Summary of Dark Data}
...
A number of resources are available for further \gls{information} on how dark relates to data safety:
\begin{itemize}
...
``Dazzle data''\index{Data!Dazzle} refers to data hiding by spurious, superfluous or unwanted data; this obscuring data confuses and can reduce the ability to see the whole picture. The data which is obscured becomes `dark' because you can't see it or use it. Examples of dazzle data\index{Data!Dazzle} might be electrical noise masking a weak signal or camouflage hiding military assets. Dazzle data\index{Data!Dazzle} may be data that you don’t want, arrives more frequently than expected or data that arrives at an unexpected or inconvenient time.


At first sight it might be thought that dazzle data\index{Data!Dazzle} hiding the wanted data is just an annoyance rather than a problem; however, this can lead to prevention of use of the real data. It can hide the true picture as in ``wood for the trees'' and in the worst case create serious problems. Examples include false positive alerts that cause real alerts to be ignored as in the classic case of ``Crying Wolf''. Another example is a \gls{dos}\index{Denial of Service} attack, when servers can be blocked with useless requests that cannot be distinguished from the real requests. Data which is truly missed (e.g. masked by extensive electrical noise, or hidden by fabricated data to mask a fraud) could lead to potentially hazardous results.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth/2]{images/dazzleknowns.png}
...
\subsubsection{Data We Are Aware Of -- Recognised as Obscured}\index{Data!Dazzle}
...
\subsubsection{Data We Are Aware Of -- Not Recognised as Obscured}\index{Data!Dazzle}
...
\subsubsection{Data We Are Not Aware Of -- Recognised as Obscured}\index{Data!Dazzle}
...
\subsubsection{Data We Are Not Aware Of -- Not Recognised as Obscured}\index{Data!Dazzle}
...
where:
\begin{quotation}
  \dots the [\Glsxtrlong{cpu}] of the \glsxtrshort{adiru}\index{ADIRU} [\Glsxtrlong{adiru}] corrupted the \glsxtrshort{aoa} [\Glsxtrlong{aoa}] data. The exact nature of the corruption was that the \glsxtrshort{adiru} \glsxtrshort{cpu} erroneously re-labelled the altitude data word so that the binary data that represented 37,012 (the altitude at the time of the incident) would represent an angle of attack of 50.625 degrees. The \glsxtrshort{fcpc} [\Glsxtrlong{fcpc}] then processed the erroneously high \gls{aoa} data, triggering the high-\gls{aoa} protection mode, which sent a command to the electrical flight control system (EFCS) to pitch the nose down.
...
\index{Data!Dazzle}
\index{Dark Data!Comparison with Dazzle Data}\index{Dark Data}
Fundamentally, dark data\index{Dark Data} is data that is not available, but nevertheless is important, and indeed, in some cases, more important than the data that is available.
...
The opposite attributes of dark data\index{Dark Data} and dazzle data\index{Data!Dazzle} might be termed Brightness and Clarity respectively. These are potential new properties or meta-properties of data.

\autoref{tab:DarkDazzleComparison} illustrates the properties which can be affected by dark data\index{Dark Data} and dazzle data\index{Data!Dazzle} issues.
...
\begin{longtable}{|L{\dsiwgColumnWidth{0.17}}|L{\dsiwgColumnWidth{0.17}}|C{\dsiwgColumnWidth{0.17}}|C{\dsiwgColumnWidth{0.17}}|L{\dsiwgColumnWidth{0.32}}|}
  \caption{\Glsentryplural{data property} affected by dark data\index{Dark Data} and / or dazzle data\index{Data!Dazzle} issues}
  ...
  \endfirsthead
  \caption[]{\Glsentryplural{data property} affected by dark data\index{Dark Data} and / or dazzle data issues (continued)}
  \\\hline\TableHeadColour{\Glsentrytext{data property}} & \TableHeadColour{Explanation} & \TableHeadColour{Dark Data -- missing data} & \TableHeadColour{Dazzle data -- obscuring data} & \TableHeadColour{Notes}\\\hline
  \endhead
  \multicolumn{5}{r}{\sl Continued on next page}
  \endfoot\endlastfoot
  %
  \Gls{integrity}\index{Integrity!Property} (I) & The data is correct, true and unaltered & \tick & \tick &
  ...
  \hline
  %
  \Gls{completeness}\index{Completeness!Property}\index{Data Property!Completeness} (C) & The data has nothing missing or lost & \tick & - &
  ...
  \hline
  %
  \Gls{consistency}\index{Consistency!Property} (N) & The data adheres to a common world view (e.g., units) & - & - &\\
  \hline
  %
  \Gls{continuity}\index{Continuity Property}\index{Data Property!Continuity} (Y) & The data is continuous and regular without gaps or breaks & \tick & - &
  ...
  \hline
  %
  \Gls{accuracy}\index{Accuracy Property}\index{Data Property!Accuracy} (A) & The data has sufficient detail for its intended use & \tick & - &
  If the data has lost detail (e.g., due to sensor sampling or representation) then this is a dark case.\\
  \hline
  %
  \Gls{resolution}\index{Resolution Property} (R) & The smallest difference between two adjacent values that can be represented
  ...
  \hline
  %
  \Gls{traceability}\index{Traceability Property} (T) & The data can be linked back to its source or derivation & \tick & - &
  ...
  \hline
  %
  \Gls{timeliness}\index{Timeliness Property} (M) & The data is as up to date as required & \tick & \tick &
  If data is not, or thought to be not, up to date then it may be rejected and hence become dark data\index{Dark Data}. But clocks can become out of sync across different systems leading to erroneous rejection so the timestamps may then be thought of as dazzling the system.\\
  \hline
  %
  \Gls{verifiability}\index{Verifiability Property} (V) & The data can be checked and its properties demonstrated to be correct & \tick & - &
  If data can’t be checked then it may be rejected (and become dark), even if it is valid.\\
  \hline
  %
  \Gls{availability}\index{Availability Property}\index{Data Property!Availability} (L) & The data is accessible and usable when an authorised entity demands access & \tick & - &
  If data is not available as required then it is dark (whether it exists or not).\\
  \hline
  %
  \Gls{fidelity}\index{Fidelity Property} (F) & How well the data maps to the real-world entity it is trying to model &
  - & - &\\
  \hline
  %
  \Gls{priority}\index{Priority Property} (P) & The data is presented / transmitted / made available in the order required &
  \tick & \tick &
  ...
  \hline
  %
  \gls{sequencing}\index{Sequencing Property} (Q) & The data is preserved in the order required &
  \tick & \tick &
  ...
  \hline
  %
  \Gls{intended_destination}\index{Intended Destination Property} (U) & The data is only sent to those that should have access to it &
  \tick & - &
  If data is not sent to the correct destination it may be a dark data\index{Dark Data} situation as it is effectively missing.\\
  \hline
  %
  \Gls{accessibility} \index{Accessibility Property}\index{Data Property!Accessibility}(B) & The data is visible only to those that should see it &
  - & \tick &
  ...
  \tick & \tick &
  If the \gls{lifetime} erroneously ends too early then the data is lost -- a dark data\index{Dark Data} problem. Conversely if the \gls{lifetime} is too long it is potentially a dazzle situation.\\
  \hline
  %
  \Gls{disposability}\index{Disposability Property} (D) & The data can be permanently removed when required &
  - & \tick &
  ...
  \tick & \tick &
  Too little data may be a dark data\index{Dark Data} case and too much data to deal with is a dazzle case.\\
  \hline
  %
  \Gls{analysability}\index{Analysability Property}\index{Data Property!Analysability} (Z) & The data is of a suitable size, type and representation (including any \gls{metadata}) to enable it be usefully analysed &
  \tick & \tick &
  If the data can’t be analysed it may not be able to be used causing a dark data\index{Dark Data} issue. If there is too much of it or it is obscured (e.g., by noise on a comms line) then it is a dazzle case.\\
  \hline
  %
  ...
  \hline
  %
  \Gls{clarity}\index{Clarity Property}\index{Data Property!Clarity} (K) & The data is visible, unmasked, not hidden or obscured, and not camouflaged &
  \tick & \tick &
  Clarity covers the cases where data is invisible or not included for some reason, often because it is hidden by other data. This could be due to either.\\
  \hline
\end{longtable}
