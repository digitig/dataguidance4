...
The \gls{odr} assessment (Part 2 Section~\ref*{Part2-bkm:assessment}) can be used to form a high-level understanding of each organization's risk. It can be used at programme level to cover all \index{Stakeholder}\glspl{stakeholder} or used by each individual \index{Stakeholder}\gls{stakeholder} to determine the
...
The process of establishing the internal context includes understanding organizational culture. A short data safety culture questionnaire is given in Part 2 Section~\ref*{Part2-bkm:culture}, which may help. It can be applied at an organization level or, more likely, within an individual project team. It could also be used to highlight the importance of data safety related issues within a project team, and before and after measurements could be taken to establish the effectiveness of data safety related training\index{Training!Personnel}.
...
Planning of the assessment may be done through a procurement process. Procurers may wish to know their potential supplier's understanding of data safety and their plans to implement a \index{Safety Assessment!Data}\gls{safety assessment}. A data safety supplier questionnaire is given in Part 2 Section\ref*{Part2-bkm:maturity}) and may be used to determine the potential supplier's understanding and plans, and for auditing purposes.
...
Part 2 Section~\ref{Part2-bkm:accidents}. Each domain might have its own catalogue of historical incidents, which can be consulted during a data-focused review.
...
\Gls{tailoring}\index{Tailoring} of the bottom-up approach may also be appropriate for some high-risk (ODR4) systems, but in this case an explicit argument that the \gls{tailoring}\index{Tailoring} has not adversely affected system safety would be expected. Furthermore, the risk identification process for high-risk (ODR4) systems is expected to be highly structured. To support this, a number of data-related \gls{hazop}\index{HAZOP} guidewords are presented in Part 2 Chapter~\ref*{Part2-bkm:guidewords}.
...
Part 2 Subsection~\ref*{Part2-bkm:Establishing-DSALS}\index{Assurance Level!Data}
and Part 2 Table~\ref*{Part2-tab:Likelihood}, while default definitions for severity are presented in Part 2 Table~\ref*{Part2-tab:Severity}.
