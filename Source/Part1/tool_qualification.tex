...
consider every new application as a new use of the tool -- in other words, classification of a tool can never be truly generic, but must be reviewed for each specific use. 
  An example of failing to carry out this reassessment is the Covid Summary Table (see Part 2 Section~\ref*{Part2-bkm:incacc:covidexcel}),
...
\item When looking to the risks of tools, it is not sufficient to consider only the functional risks like ``wrong data created''
  ...
  (Part 2 Subsection~\ref*{Part2-tab:issues} Ways that Data Can Cause  Problems) should be considered for impact from the tools.
  ...
  \begin{itemize}
  \item Each issue listed in Part 2 Subsection~\ref*{Part2-tab:issues}
  \item Each Property listed in Part 2 Table \ref*{Part2-tab:PropertiesOfData}
  \end{itemize}    
\end{itemize}
