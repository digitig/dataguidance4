...
%
\subsubsection{The \glsfmttext{analysability}\index{Analysability Property} property}\label{bkm:guidance:analysability}
...
\subsubsection{The \glsfmttext{explainability}\index{Explainability Property} property}\label{bkm:guidance:explainability}
...
\begin{longtable}{|L{\dsiwgColumnWidth{0.24}}|C{\dsiwgColumnWidth{0.15}}|L{\dsiwgColumnWidth{0.61}}|}
  ...
  {\Gls{intended_destination}}\index{Data Property!Intended Destination|see {Intended Destination Property}}\index{Intended Destination Property} & U & {The data is only sent to those that should have
  access to
  it}\\\hline
...
\begin{itemize}
    ...
    \item Determine the list of applicable \index{Data Property}\glspl{data property}, using the abbreviations in \autoref{tab:PropertiesOfData}. This will generally provide a list of several applicable abbreviations.
\end{itemize}
...
\begin{itemize}
    ...
    \item The \index{Artefact, Data}\gls{data artefact} holds one or more of the properties\index{Integrity!Property}\gls{integrity}, \index{Completeness!Property}\gls{completeness} or \gls{verifiability}\index{Verifiability Property} (``I'', ``C'' or ``V'' from \autoref{tab:PropertiesOfData}).
\end{itemize}
...
Many dots have been placed in the table to indicate data categories\index{Category!Data} and \index{Data Property}data properties which are not applicable to the technique under consideration. The dots enable the tables to be assessed very quickly, as they enable the letters representing specific data categories\index{Category!Data} and \index{Data Property}\glspl{data property} to always be presented in the same position within the table. For example, the \index{Data Property}\gls{data property} ``\gls{verifiability}''\index{Verifiability Property} (``V'' from \autoref{tab:PropertiesOfData} can easily be seen to appear against the first two techniques of \autoref{tab:MethodsSystemDesign}, whereas it does not appear in the next seven techniques. This can be seen without actually reading the text, but merely looking at the pattern presented by the letters and dots. 
