...
%
%================================================================================
\chapter{Worked Example} \label{bkm:workedexample}
...
The manufacturer considered each of the questions within the \gls{odr} form from Chapter~\autoref{bkm:assessment} of this document.
...
The final score is \dsiwgTextBF{26}, which corresponds to \dsiwgTextBF{ODR2}. The manufacturer therefore concludes that there is low to medium risk that loss of \index{Data Property}properties of data in the system can contribute to or give rise to harm. The manufacturer has an internal policy for engagements based on the \gls{odr} level that dictates how the organization will plan the \cbstart safety management\cbend. This policy dictates the amount of proportional effort the manufacturer needs to spend on safety data management and the level of rigour to be employed. In this case, the policy dictates, amongst other requirements, that a separate section covering safety data management is required in its clinical risk management plan.
...
The health organization aims to plan the \cbstart safety management\cbend\ through a clinical risk management plan. This plan defines the organization and system context in more detail and lays out the planned activities for identifying, evaluating and treating data safety related risks.
...
\gls{verification}\index{Verification Data} and dynamic\index{Dynamic Data} lifecycle data categories to be used
during these phases.
...
To analyse the risks in more detail, the manufacturer uses a top-down approach and a bottom-up approach. In the first approach it considers each of the system functions (such as clinical screens) and analyses where there is a dependency on data and what \index{Data Property}properties need to be preserved. In the second, as much of the functionality is driven by data flows in and out of the system, the manufacturer also looks at specific data flows and assesses the impact of loss of \index{Data Property}properties for the data in those flows.
...
As with the manufacturer, both a top-down approach and a bottom-up approach are adopted. From the health organization's perspective, one key focus for \gls{hazard} identification is in the use of dynamic data, i.e., data that will be delivered into the new system from existing system data sources and the data presented to the user. For the interactions identified in the supply chain, the health organization needs to consider the risks associated with loss of \index{Data Property}properties of the data it will receive. Questions the health organization will need to consider and address more formally in the clinical risk management plan are:
\clearpage% Required on 3.3 to keep page breaks as 3.2
\begin{itemize}
  \item which \glspl{dataset} or \glspl{item data}\ being received from other systems have \index{Data Property}\glspl{data property} (such as \gls{timeliness}\index{Timeliness Property}, \index{Completeness!Property}\index{Data Property!Completeness}\gls{completeness}, \index{Consistency!Property}\gls{consistency}, \index{Fidelity Property}\gls{fidelity} etc.) that are significant to patient safety? 
  \item what data presented to the user has \index{Data Property}\glspl{data property} (such as \index{Availability Property}\index{Data Property!Availability}\gls{availability}, format\index{Format Property}, \gls{resolution}\index{Resolution Property}, etc.) that are significant to patient safety?
  \item what existing barriers or \glspl{mitigation}\index{Mitigation} (physical, technical, procedural) exist to reduce the risk of loss of \index{Data Property}\glspl{data property}? and
...
In the second phase of the development lifecycle\index{Lifecycle!Development}, where \gls{verification} data\index{Verification Data} and infrastructure data\index{Infrastructure Data} is being used to demonstrate the correct functioning of the system, the manufacturer considers that loss of any of the \glspl{data property} of \index{Integrity!Property}\gls{integrity}, \index{Completeness!Property}\index{Data Property!Completeness}\gls{completeness}, \index{Consistency!Property}\gls{consistency}, \index{Continuity Property}\index{Data Property!Continuity}\gls{continuity}, \index{Format Property}\gls{format}, \index{Accuracy Property}\index{Data Property!Accuracy}\gls{accuracy}, \gls{resolution}\index{Resolution Property}, \gls{timeliness}\index{Timeliness Property}, \index{Availability Property}\index{Data Property!Availability}\gls{availability}, \index{Fidelity Property}\gls{fidelity}, \gls{sequencing}\index{Sequencing Property}, \gls{intended_destination}\index{Intended Destination Property}, \gls{goldilocks}\index{Goldilocks Property}, and explainability\index{Explainability Property} of this data could give rise to \glspl{hazard}.

For example, if the \gls{verification} \gls{dataset} selected is not representative of the eventual diversity experienced in practice (loss of \index{Fidelity Property}\gls{fidelity}), then it is possible that the system may contain latent software errors that could give rise to harm. However, the manufacturer acknowledges that the system will be subject to further testing and trials in the clinical setting and so there will be other opportunities to detect errors in the system. Overall:
\begin{itemize}
...
The manufacturer therefore assesses these lifecycle data categories\index{Category!Data} as \dsiwgTextBF{DSAL1}\index{Assurance Level!Data} in this particular context of use.
...
\begin{itemize}
  \item how likely is it that there would be a loss of the given \index{Data Property}\gls{data property}?
  \item how would such a loss of a \index{Data Property}\glspl{data property} be detected?
...
Concerns were raised about the vulnerability of the system to inadvertent data overload from the large number of potential data sources, and possibly from malicious activity. This led to the addition of the \gls{goldilocks}\index{Goldilocks Property} property to the list of \index{Data Property}\glspl{data property} applicable to the live system.
...
\begin{itemize}
  \item The health organizations assesses the likelihood of this loss of \index{Data Property}property as \dsiwgTextBF{High}; and
...
For \dsiwgTextBF{DSAL1}\index{Assurance Level!Data} \gls{verification} data, the tables show that the following are recommended (R) or highly recommended (HR) where loss of \glspl{data property} 
	...
	\gls{continuity}\index{Continuity Property}\index{Data Property!Continuity},
	\gls{format}\index{Format Property},
	\gls{accuracy}\index{Accuracy Property}\index{Data Property!Accuracy},
	\gls{resolution}\index{Resolution Property},
	\gls{timeliness}\index{Timeliness Property},
	\gls{availability}\index{Availability Property}\index{Data Property!Availability},
	\gls{fidelity}\index{Fidelity Property},
	\gls{sequencing}\index{Sequencing Property},
	\gls{intended_destination}\index{Intended Destination Property},
...
\begin{longtable}{|C{\dsiwgColumnWidth{0.08}}|L{\dsiwgColumnWidth{0.15}}|C{\dsiwgColumnWidth{0.08}}|L{\dsiwgColumnWidth{0.69}}|}
  ...
  \hfill(extracted from Table~\ref*{tab:MethodsSystemDesign})
  }\\\hline
  \endfirsthead
...
\addtocounter{table}{-1} % Kludge because longtable* is wrongly incrementing the table counter
\begin{longtable*}{|C{\dsiwgColumnWidth{0.08}}|L{\dsiwgColumnWidth{0.15}}|C{\dsiwgColumnWidth{0.08}}|L{\dsiwgColumnWidth{0.69}}|}
  \TableHeadColourC{Ref} & \TableHeadColour{Technique} & \TableHeadColourC{R / HR} & \TableHeadColour{Data Design
  \hfill(extracted from Table~\ref*{tab:MethodsDataDesign})}\\
  \hline
  \endfirsthead
  \TableHeadColourC{Ref} & \TableHeadColour{Technique} & \TableHeadColourC{R / HR} & \TableHeadColour{Data Design}\\\hline
  \endhead
  \endfoot\endlastfoot
...
\addtocounter{table}{-1} % Kludge because longtable* is wrongly incrementing the table counter
\begin{longtable*}{|C{\dsiwgColumnWidth{0.08}}|L{\dsiwgColumnWidth{0.15}}|C{\dsiwgColumnWidth{0.08}}|L{\dsiwgColumnWidth{0.69}}|}
  \TableHeadColourC{Ref} & \TableHeadColour{Technique} & \TableHeadColourC{R / HR} & \TableHeadColour{Data Implementation
  \hfill(extracted from Table~\ref*{tab:MethodsDataProcedures})
  }\\\hline
  \endfirsthead
  \TableHeadColourC{Ref} & \TableHeadColour{Technique} & \TableHeadColourC{R / HR} & \TableHeadColour{Data Implementation}\\\hline
  \endhead
  \endfoot\endlastfoot
...
\addtocounter{table}{-1} % Kludge because longtable* is wrongly incrementing the table counter
\begin{longtable*}{|C{\dsiwgColumnWidth{0.08}}|L{\dsiwgColumnWidth{0.15}}|C{\dsiwgColumnWidth{0.08}}|L{\dsiwgColumnWidth{0.69}}|}
  \TableHeadColourC{Ref} & \TableHeadColour{Technique} & \TableHeadColourC{R / HR} & \TableHeadColour{Data Migration
  \hfill(extracted from Table~\ref*{tab:MethodsDataMigration})
  }\\\hline
  \endfirsthead
  \TableHeadColourC{Ref} & \TableHeadColour{Technique} & \TableHeadColourC{R / HR} & \TableHeadColour{Data Migration}\\\hline
  \endhead
  \endfoot\endlastfoot
...
\addtocounter{table}{-1} % Kludge because longtable* is wrongly incrementing the table counter
\begin{longtable*}{|C{\dsiwgColumnWidth{0.08}}|L{\dsiwgColumnWidth{0.15}}|C{\dsiwgColumnWidth{0.08}}|L{\dsiwgColumnWidth{0.69}}|}
  \TableHeadColourC{Ref} & \TableHeadColour{Technique} & \TableHeadColourC{R / HR} & \TableHeadColour{Data Checking
  \hfill(extracted from Table \ref*{tab:MethodsDataChecking})
  }\\\hline
  \endfirsthead
  \TableHeadColourC{Ref} & \TableHeadColour{Technique} & \TableHeadColourC{R / HR} & \TableHeadColour{Data Checking}\\\hline
  \endhead
  \endfoot\endlastfoot
  \multicolumn{4}{|l|}{No relevant techniques}\\
  \hline
...
\addtocounter{table}{-1} % Kludge because longtable* is wrongly incrementing the table counter
\begin{longtable*}{|C{\dsiwgColumnWidth{0.08}}|L{\dsiwgColumnWidth{0.15}}|C{\dsiwgColumnWidth{0.08}}|L{\dsiwgColumnWidth{0.69}}|}
  \TableHeadColourC{Ref} & \TableHeadColour{Technique} & \TableHeadColourC{R / HR} & \TableHeadColour{Test Data
  \hfill(extracted from Table~\ref*{tab:MethodsTestData})
  }\\\hline
  \endfirsthead
  \TableHeadColourC{Ref} & \TableHeadColour{Technique} & \TableHeadColourC{R / HR} & \TableHeadColour{Test Data}\\\hline
  \endhead
  \endfoot\endlastfoot
...
\addtocounter{table}{-1} % Kludge because longtable* is wrongly incrementing the table counter
\begin{longtable*}{|C{\dsiwgColumnWidth{0.08}}|L{\dsiwgColumnWidth{0.15}}|C{\dsiwgColumnWidth{0.08}}|L{\dsiwgColumnWidth{0.69}}|}
  \TableHeadColourC{Ref} & \TableHeadColour{Technique} & \TableHeadColour{R / HR} & \TableHeadColour{Media -- paper
  \hfill(extracted from Table~\ref*{tab:MethodsDataMediaPhysical})
  }\\\hline
  \endfirsthead
  \TableHeadColourC{Ref} & \TableHeadColour{Technique} & \TableHeadColour{R/HR} & \TableHeadColour{Media -- paper}\\\hline
  \endhead
  \endfoot\endlastfoot
...
\addtocounter{table}{-1} % Kludge because longtable* is wrongly incrementing the table counter
\begin{longtable*}{|C{\dsiwgColumnWidth{0.08}}|L{\dsiwgColumnWidth{0.15}}|C{\dsiwgColumnWidth{0.08}}|L{\dsiwgColumnWidth{0.69}}|}
  \TableHeadColourC{Ref} & \TableHeadColour{Technique} & \TableHeadColourC{R / HR} & \TableHeadColour{Media -- electronic
  \hfill(extracted from Table~\ref*{tab:MethodsDataMediaElectronic})
  }\\\hline
  \endfirsthead
  \TableHeadColourC{Ref} & \TableHeadColour{Technique} & \TableHeadColourC{R/HR} & \TableHeadColour{Media -- electronic}\\\hline
  \endhead
  \endfoot\endlastfoot
