...
\glsxtrshort{scsc} Publication Number: SCSC-127K
...
Front cover design by DeepAI.org and Tim Rowe, based on an original by Paul Hampton.
Back cover design by vector\_corp on Freepik and Tim Rowe
...
\gls{sss}'26, 10--12 February 2026,
details of which may be found at
\href{https://scsc.uk/sss}.
...
\begin{longtable}
	...
	\\\hline
	{4.0} & {The \gls{dsiwg} Team} & {For publication at \gls{sss}'26} & {10-FEB-2026}
	\\\hline
\end{longtable}
%
...
The process remains substantially the same. The intention is that conformance with this version does imply conformance with earlier versions , but some material that was informative\index{Informative Text} in earlier versions has been made normative\index{Normative Text}, so conformance with earlier versions of the guidance does not guarantee conformance with this version.
...
Proposed future work includes elaboration and expansion of scenarios to enable different routes into this guidance. An appendix of scenarios is included as a starting point at \dsiwgExtRef{Appendix}{2}{bkm:scenario}.
...
These improvements are intended to clarify the application of the data safety process and include:
\begin{itemize}
	...
	\item further explanation of some \glspl{treatment}, where their use or benefit is not immediately apparent; and 
	\item further elaboration of issues related to machine learning, \glspl{llm} and generative \gls{ai}.
	\end{itemize}
...
Related \gls{scsc} working groups include:
%
\begin{itemize}
...
The use of \gls{ai} systems is a new, exciting, but potentially hazardous use of data. \Gls{llm} based systems are trained on data, and it is this data which enables them to be useful. Although \gls{ml} systems tend to be trained on specifically curated data, these can also be subject to issues such as bias, poisoning and omission.
...
In particular, they must actively manage key \index{Data Property}\glspl{data property} that preserve safety.

\dsiwgTextBF{Data is growing}: There are many reasons why the use of data has grown and, equally important, why it is expected to continue to grow. The  first relates to the rapid expansion \gls{ai}, particularly \glspl{llm} which are trained on vast amounts of data. A second area is ``Big Data''. A further area is the growing use of systems-of-systems, where data is the lifeblood that connects together disparate elements and allows a cohesive capability to be built. Put simply, the need to address data-related issues is a pressing problem and will continue to be.

\dsiwgTextBF{Data is causing harm}: Strictly speaking,
data can neither cause nor prevent harm.
However, mistakes in data or inappropriate uses of data within safety-related systems have been factors in a number of documented accidents and incidents. Examples include aircraft attempting to take off from the wrong runway (and consequently crashing), ships running aground, and patients being exposed to higher than planned doses of radiation.
...
%When reference is made to the content of other volumes, this is indicated by the volume number followed by the reference within that volume in parentheses. So ``\dsiwgExtRef{Chapter}{2}{bkm:DsalCustomisation}'' refers to chapter \ref*{Part2-bkm:DsalCustomisation} in volume 2.
...
\begin{description}
	\item[Volume 1] contains the normative\index{Normative Text} material. It describes the ``what'' of the guidance and states what is needed to ensure conformance with this guidance, if required. It should be of use to those establishing processes and procedures for data safety assurance and for those doing the actual assurance;
	\item[Volume 2] contains the informative\index{Informative Text} material. It describes the ``how'' of the guidance and should be of particular value to data safety practitioners;
	\item[Volume 3] (this volume) contains the discursive\index{Discursive Text} material. It describes the ``why'' of the guidance and gives background and supporting material intended to improve practitioner's understanding of the data safety management processes and of issues relating to data safety management.
\end{description}
	
\begin{itemize}
	\item The data properties that need to be guaranteed to ensure system safety are described in \dsiwgExtRef{Chapter}{1}{bkm:dataProperties}
	\item The underlying principles of the guidance are described in \dsiwgExtRef{Chapter}{1}{bkm:principles}.
	\item The objectives and outputs to satisfy the principles are described in \dsiwgExtRef{Chapter}{1}{bkm:objectives}.
	\item Definitions and abbreviations are described in \dsiwgExtRef{Chapter}{1}{bkm:definitionsabbreviations}.
	\item A possible data safety management process is outlined in \dsiwgExtRef{Chapter}{2}{bkm:process}
	\item The activities of each phase (and associated \gls{tailoring} \gls{information}) are described in \dsiwgExtRef{Chapter}{2}{bkm:activities}.
	\item More detailed guidance is given in \dsiwgExtRef{Chapter}{2}{bkm:guidance}
	\item A worked example is provided in \dsiwgExtRef{Chapter}{2}{bkm:workedexample}.
	\item Guidance for the assignment of \glspl{dsal} is given in \dsiwgExtRef{Chapter}{2}{bkm:DsalCustomisation}.
	\item Tool qualification is addressed in \dsiwgExtRef{Chapter}{2}{bkm:tools}.
	\item Issues relating to \gls{ai} and \gls{ml} are considered in \dsiwgExtRef{Chapter}{2}{bkm:MachineLearning} and \dsiwgExtRef{Chapter}{2}{bkm:autonomy}.
	\item Additional information on lifecycle data categories is given in \dsiwgExtRef{Chapter}{2}{bkm:categories}.
	\item Issues that may arise when migrating, porting, importing or exporting data are discussed in \dsiwgExtRef{Chapter}{2}{bkm:migration}).
	\item Rare events are addressed in \dsiwgExtRef{Chapter}{2}{bkm:cygnology}.
	\item The relationship between typical risk-inducing issues and the data properties most likely to be affected are discussed in \dsiwgExtRef{Chapter}{2}{bkm:additionalways}
	\item Various supplemental material is given in appendices to Volume 2.
	\item Lifecycle considerations are addressed in \dsiwgRef{Chapter}{bkm:lifecycle}.
	\item The data safety tool RADISH is described in \dsiwgRef{Chapter}{bkm:radish}.
	\item Some of the data issues that made management of the Covid-19 virus difficult are discussed in \dsiwgRef{Chapter}{bkm:Covid19}.
	\item A summary of accidents and incidents in which data was potentially a causal factor is given in Chapter \dsiwgRef{Chapter}{bkm:accidents};
\end{itemize}
