...
Proposed future work includes elaboration and expansion of scenarios to enable different routes into this guidance. An appendix of scenarios is included as a starting point at \dsiwgRef{Appendix}{bkm:scenario}.
...
\begin{description}
	...
	\item[Volume 2] (this volume) contains the informative\index{Informative Text} material. It describes the ``how'' of the guidance and should be of particular value to data safety practitioners;
...
\begin{itemize}
	...
	\item The activities of each phase (and associated \gls{tailoring} \gls{information}) are described in \dsiwgRef{Chapter}{bkm:activities}.
	\item More detailed guidance is given in \dsiwgRef{Chapter}{bkm:guidance}
	\item A worked example is provided in \dsiwgRef{Chapter}{bkm:workedexample}.
	\item Guidance for the assignment of \glspl{dsal} is given in \dsiwgRef{Chapter}{bkm:DsalCustomisation}.
	\item Tool qualification is addressed in \dsiwgRef{Chapter}{bkm:tools}.
	\item Issues relating to \gls{ai} and \gls{ml} are considered in \dsiwgRef{Chapter}{bkm:MachineLearning} and \dsiwgRef{Chapter}{bkm:autonomy}.
	\item Additional information on lifecycle data categories is given in \dsiwgRef{Chapter}{bkm:categories}.
	\item Issues that may arise when migrating, porting, importing or exporting data are discussed in \dsiwgRef{Chapter}{bkm:migration}).
	\item Rare events are addressed in \dsiwgRef{Chapter}{bkm:cygnology}.
	\item The relationship between typical risk-inducing issues and the data properties most likely to be affected are discussed in \dsiwgRef{Chapter}{bkm:additionalways}
	\item Various supplemental material is given in appendices to this volume.
