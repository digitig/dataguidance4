...
The \gls{odr} can be used to help identify additional\hl{???Additional to what?} key \index{Stakeholder}\glspl{stakeholder} and necessary approvers (i.e., those who need to formally accept the system). Some approvers might be within the organization, while others may represent external bodies. Approvers could be customers or regulatory authorities.
...
\subsubsection{Data safety requirements}\hl{???need more more content}
Different levels of risk may be associated with composites of safety-related data, which may be easier to manage than individual \index{Artefact, Data}\glspl{data artefact}, or where independence cannot be demonstrated or maintained. Hence, the partitioning of \glspl{dataset} should also be considered during this phase.
\subsubsection{Data safety requirement management}\hl{???Need content. Note that data safety management comes up again in the next phase}
...
\subsubsection{Avoiding risks}
\hl{???Need text}
\subsubsection{Accepting risks}
\hl{???Need text}
\subsubsection{Transferring risk}
\hl{???Need text}
\subsubsection{Treating risk}
If a decision is made to \gls{treat} a risk, suitable methods and approaches should be identified. A range of potential methods and approaches are included in this guidance. These are mapped against \glspl{dsal}\index{Assurance Level!Data}, \index{Data Property}\glspl{data property} and a selection of lifecycle\index{Lifecycle!Data} data categories.
