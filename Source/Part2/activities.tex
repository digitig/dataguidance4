...
Details of the planned assessment may be recorded in a \gls{dsmp} \cbstart(The \gls{dsmp} may in turn be part of a higher-level planning document such as a project plan or project  safety management plan is used for the project, data safety aspects may be included in the project safety management plan)\cbend. This could also be used to capture the scope of the analyses and the associated context. Together, this constitutes the first section of the \gls{dsmp}. Note that if a \gls{dsmp} is used, it would typically be updated with details from subsequent phases. 

Planning of the \index{Safety Assessment!Data}\gls{safety assessment} requires some knowledge of the quantity and complexity of data that requires assessment. Therefore, the \gls{dsmp} needs to be updated in subsequent phases once these details are known.
