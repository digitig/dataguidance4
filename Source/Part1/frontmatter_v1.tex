...
The process remains substantially the same. The intention is that compliance with this version does imply compliance with earlier versions \hl{???Need to check this is still true}, but some material that was informative in earlier versions has been made normative, so compliance with earlier versions of the guidance does not guarantee compliance with this version.\cbend\
...
When reference is made to the content of other volumes, this is indicated by the volume number shown in bold text. So ``\dsiwgExtRef{Chapter}{3}{Part3-bkm:accidents}'' refers to chapter \ref*{Part3-bkm:accidents} in volume 3.
...
\begin{itemize}
	...
	\item Normative definitions and abbreviations are described in \dsiwgRef{Chapter}{bkm:definitionsabbreviations}.
