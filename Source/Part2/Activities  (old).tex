...
%
%================================================================================
\cbstart\chapter{Activities}\label{bkm:activitiestailoring}\cbend
...
Part of this activity involves understanding the system \index{Stakeholder}\glspl{stakeholder}. It is important to define how the \index{Stakeholder}\glspl{stakeholder} will interact, determine the derived requirements applicable to each \index{Stakeholder}\gls{stakeholder}, and identify how these requirements will be managed through organizational interface control\index{Interface!organizational} (similarly to systems engineering interface control procedures already in place in many industries). External factors (e.g., economic, social, regulatory) and internal factors (e.g., cultural, processes, strategic) are key to defining appropriate organizational interface control measures. Organizational interface control may be iterative throughout the \cbstart data safety management process\cbend. In particular, organizational interface control may need to be amended to take account of implemented data safety \glspl{mitigation}\index{Mitigation}.

The \gls{odr} assessment (Section~\ref{bkm:assessment}) can be used to form a high-level understanding of each organization's risk. It can be used at programme level to cover all \index{Stakeholder}\glspl{stakeholder} or used by each individual \index{Stakeholder}\gls{stakeholder} to determine the
risk appropriate to their area. However, when adopting such a formulaic approach to risk \cbstart management\cbend, care should be taken 
to ensure that the resulting system can meet its cumulative requirements and that nothing has ``fallen through the cracks''. This is part of the requirements decomposition and \gls{verification} / \gls{validation} activities described later in the data safety management process.

Among other things, the \gls{odr} assessment considers:
\begin{itemize}
	\item the severity of any potential accidents;
...
The \gls{odr} can be used to help identify additional key \index{Stakeholder}\glspl{stakeholder} and necessary approvers (i.e., those who need to formally accept the system). Some approvers might be within the organization, while others may represent external bodies. Approvers could be customers or regulatory authorities.

The process of establishing the internal context includes understanding organizational culture. A short data safety culture questionnaire is given in Section~\ref{bkm:culture}, which may help. It can be applied at an organization level or, more likely, within an individual project team. It could also be used to highlight the importance of data safety related issues within a project team, and before and after measurements could be taken to establish the effectiveness of data safety related training\index{Training!Personnel}.
...
As development progresses, the system description is likely to be refined. This may enable refinement of the data safety system context, supporting the \cbstart data safety management\cbend\ planning.

\subsubsection{Plan the safety management}
...
Details of the planned \cbstart safety management\cbend\ may be recorded in a \gls{dsmp} \cbstart(The \gls{dsmp} may in turn be part of a higher-level planning document such as a project plan or project safety management plan)\cbend. This could also be used to capture the scope of the analyses and the associated context. Together, this constitutes the first section of the \gls{dsmp}. Note that if a \gls{dsmp} is used, it would typically be updated with details from subsequent phases. 

Planning of the \cbstart data safety management\cbend\ requires some knowledge of the quantity and complexity of data that requires \cbstart management\cbend. Therefore, the \gls{dsmp} needs to be updated in subsequent phases once these details are known.

Planning of the \cbstart data safety management\cbend\ may be done through a procurement process. Procurers may wish to know their potential supplier's understanding of data safety and their plans to implement a \index{Safety Assessment!Data}\gls{data safety assessment}. A data safety supplier questionnaire is given in Section\ref{bkm:maturity} and may be used to determine the potential supplier's understanding and plans, and for auditing purposes.
...
Including data safety within a project safety management plan is recommended for complex or highly safety-critical systems. In this case the structure of the safety management plan may be maintained, with the \index{Safety Assessment!Data}\gls{data safety assessment} process being tailored to align to the overall \cbstart safety management process\cbend.

\cbstart A data maturity questionnaire (\autoref{bkm:maturity}) is likely to be useful, particularly\cbend\ when new organizational relationships are being formed. It may offer little value in situations where both organizations are familiar with each other, they have worked on data-related projects together before and there are suitable audit / review arrangements in place.
...
Part 3 Section~\ref{Part3-bkm:accidents}. Each domain might have its own catalogue of historical incidents, which can be consulted during a data-focused review. Some organizations carry out a ``lessons learned'' review at the end of projects, which can also give insight into potential risks.
...
\glspl{dsal} are measured on a scale of DSAL0 (lowest-assurance) to DSAL4 (highest-assurance). They are typically allocated as indicated in a form similar to  \autoref{tab:DSAL-risk-matrix}. \cbstart The specific allocation of \glspl{dsal} to likelihoods and severities will vary between industries and applications, and is discussed further in \hl{???insert reference}.\cbend
...
As their name suggests, \glspl{dsal} are focused on safety concerns. However, the framework of \index{Artefact, Data}\glspl{data artefact}, \index{Data Property}\glspl{data property}, and so on, developed in this document could also be applied to other concerns. It could, for example, be used for to control data-related financial risks, or data-related reputational risks. In these types of approach, the severity terms would obviously relate to financial and reputational consequences, rather than safety ones.
...
This activity involves comparing \glspl{dsal}\index{Assurance Level!Data} with \gls{integrity}\index{Integrity}, or assurance, levels. Assuming a typical scenario of a system processing or manipulating data flowing through it, there are two cases to consider:
...
\begin{description}
  ...
  \item[Transfer]Ownership of the risk's consequence can be transferred, for example to another organization \cbstart(for example by taking out an insurance policy) or to another part of the system that will address the risk\cbend. It is important that any such risk transfers are formally documented, understood and agreed by both parties.
...
A significant amount of \gls{tailoring}\index{Tailoring} is implicit in the way the tables of methods and approaches \cbstart in Chapter~\ref{bkm:activitiestailoring}\cbend\ are constructed. At best a method / approach may be highly recommended as a way of maintaining a required \index{Data Property}data property at a given \gls{dsal}\index{Assurance Level!Data}. The tables are not exhaustive; additional or alternative methods and approaches can be used.
