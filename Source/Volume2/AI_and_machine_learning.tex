...
%
%================================================================================
\chapter{\Glsfmtshort{ai} and \ecapitalisewords{\glsentrylong{ml}}} \label{bkm:MachineLearning}
\index{AI|(}
...
\section{Lifecycle Data Categories}
Three \index{Lifecycle}lifecycle data categories are discussed: \gls{ml} training\index{Training!AI} data\index{Machine Learning Data} (the data used to train the model),
\gls{ml} test data\index{Category!Data!Verification} (used to provide a measure the trained model's \index{Accuracy Property}\index{Data Property!Accuracy}\gls{accuracy}), and \gls{ml}
...
Sensors may also suffer degradation due to ageing and specific faults due to their positioning, construction or configuration. Sensors may need to be diverse to increase resilience and give an overall more reliable output. Realistic data variations, informed by an understanding of likely sensor failure modes, need to be incorporated in the training\index{Training!AI} \gls{dataset} (c.f.\ the AoA sensor in the Boeing 737 MAX~8, discussed in Part 3 Section \ref*{Part3-bkm:incacc:737max8}).
Sensor fusion can effectively create ``composite data'', e.g.\ from merging of lidar, radar and camera data; this will have different (and possibly hard to predict) \index{Data Property}properties compared to the individual sensors.
...
\section{\index{Data Property}\ecapitalisewords{\glsentryplural{data property}}}
Bias, sufficiency and \index{Accuracy Property}\index{Data Property!Accuracy}\gls{accuracy} are all concerns with the data. \index{Fidelity Property}\gls{fidelity} may also be a problem as modelling real-life sensor data is difficult.  Errors should be introduced for training\index{Training!AI}: this may be by analytical means, by random, automated sampling of data to determine the degree of similarity between samples, or by deliberate insertion of known edge cases. 
It is suggested that all the \index{Data Property}\glspl{data property} mentioned in this guidance document are considered.
...
the preservation of \index{Data Property}\glspl{data property} as described within this document, and an update process such as that specified in ISO 21448 (SOTIF)
...
\item Quantifying Data Set \index{Data Property}Properties for Systematic Artificial Neural Network Classifier \Glsfmttext{verification} \cite{citation:Hond2020}
